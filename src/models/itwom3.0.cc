...
auto alos(double d, prop_type & prop, propa_type & propa) -> double
{
	...
	if (d == 0.0) {
		wls =
		    0.021 / (0.021 +
			     prop.wn * prop.dh / std::max(10e3, propa.dlsa));
		alosv = 0.0;
	}
...
void lrprop(double d, prop_type & prop, propa_type & propa)
{
	...
	if (prop.mdp != 0) {
		...
		propa.tha =
		    std::max(prop.the[0] + prop.the[1], -propa.dla * prop.gme);
		wlos = false;
		wscat = false;
	    ...
	    propv_type & propv) -> double
{
	...
	if (propv.lvar > 0) {
		...
		case 2:
			dexa =
			    std::sqrt(18e6 * prop.he[0]) + std::sqrt(18e6 * prop.he[1]) +
			    std::cbrt((575.7e12 / prop.wn));
		...
		if (w1) {
			...
			q = (1.0 -
			     0.8 * std::exp(-prop.dist / 50e3)) * prop.dh * prop.wn;
			sgl = 10.0 * q / (q + 13.0);
		}
	    ...
	    propa_type & propa, propv_type & propv)
{
	...
	if (prop.dl[0] + prop.dl[1] > 1.5 * prop.dist) {
		...
		for (int j = 0; j < 2; j++) {
			prop.dl[j] =
			    std::sqrt(2.0 * prop.he[j] / prop.gme) * std::exp(-0.07 *
								    std::sqrt(prop.dh /
									 std::max
									 (prop.he[j], 5.0)));
		}
	     ...
	     propa_type & propa, propv_type & propv)
{
	...
	if ((np < 1) || (pfl[1] > 150.0)) {
		...
		prop.thenr =
		    std::atan(std::max(0.0, (pfl[np + 2] - pfl[np + 1])) / pfl[1]);
...
*****************************************************************************/
{
	...
	if (static_cast<int>(q) < 0.0) {
		...
		if (prop.dist <= propa.dlsa || prop.dist <= propa.dx) {
			...
			if (static_cast<int>(prop.dl[1]) == 0.0) {
				strmode += "_Peak";
			}
			else {
				strmode += "_Diff";
