...
double h0f(double r, double et)
{
	...
	if (q != 0.0)
		h0fv =
		    (1.0 - q) * h0fv + q * 4.343 * log((a[it] * x + b[it]) * x +
						       1.0);
...
double adiff(double d, prop_type & prop, propa_type & propa)
{
	...
	if (d == 0) {
		...
		q = (wd1 +
		     xd1 / d) * std::min(((1.0 - 0.8 * exp(-d / 50e3)) * prop.dh *
				       prop.wn), 6283.2);
		wd = 25.1 / (25.1 + sqrt(q));
...
{
	static int kdv;
	static double dexa, de, vmd, vs0, sgl, sgtm, sgtp, sgtd, tgtd, gm, gp,
	    cv1, cv2, yv1, yv2, yv3, csm1, csm2, ysm1, ysm2, ysm3, csp1, csp2,
	    ysp1, ysp2, ysp3, csd1, zd, cfm1, cfm2, cfm3, cfp1, cfp2, cfp3;
